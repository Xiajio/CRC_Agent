...
flowchart TD
    ...
    F --> K["SessionStore: memory / sqlite"]
...
- `session_store = build_session_store(runtime_settings, runtime_root)`
...
- 会话元数据默认存在 `InMemorySessionStore`，进程重启即丢失；设置 `SESSION_STORE_BACKEND=sqlite` 后会镜像到 `runtime/sessions.db` 或 `SESSION_STORE_SQLITE_PATH` 指定位置，重启时 rebuild 到内存。
...
- `doctor`：只创建 BFF session。
- `patient`：创建 BFF session 后，还会通过 `PatientRegistryService.create_draft_patient()` 在 SQLite 中创建草稿患者，并把 `patient_id` 写回 session。
...
| 数据 | 位置 | 生命周期 |
| --- | --- | --- |
| session id/thread id/context_state/uploaded_assets/processed_files/pending context | `InMemorySessionStore`；可选 `SqliteSessionStore` 镜像到 `runtime/sessions.db` | 默认进程内；`SESSION_STORE_BACKEND=sqlite` 时重启后可恢复 |
| active run / run locks | `InMemorySessionStore` / `SqliteSessionStore` 进程内锁 | 只在当前进程有效，重启后清空 |
...
SQLite session 持久化只覆盖 BFF session metadata。完整对话历史仍取决于 LangGraph checkpoint 后端；如果 checkpoint 仍是 memory，重启后旧 `session_id` 可以恢复 metadata，但历史消息和 graph state 仍无法从 checkpoint 恢复。
