...
def sync_to_excel(json_data: str, excel_path: str = xlsx) -> str:
    """?????????JSON???????????????xcel?????????
    ?????????:
    - json_data: ?????????????????????????????????SON?????????????    - excel_path: Excel???????????????????????????????classification.xlsx
    ?????????:
    - ???????????????????????????
    """
    try:
        ...
        return f"?? Excel ??????????? {backup_path}"
    except Exception as e:
        return f"?? Excel ??: {str(e)}"

@tool
def query_subject_info(subject_id: int, excel_path: str = xlsx) -> str:
    """??????????????????????????????????????????xcel????????????????????????????????????
    ?????????:
    - subject_id: ??????????????????????    - excel_path: Excel???????????????????????????????classification.xlsx
    ?????????:
    - ?????????????????????????????????SON?????????????????????????????????????????????
    """
    try:
        ...
        return f"?? {excel_path} ???????? {subject_id}"
    except FileNotFoundError:
        return f"Excel ?????: {excel_path}"
    except Exception as e:
        return f"????: {str(e)}"
