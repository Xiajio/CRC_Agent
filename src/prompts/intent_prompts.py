...
- 用户："查询<case_id>号患者信息" → `case_database_query`（需要新的信息）
...
1. **imaging_analysis (影像分析)** [优先级最高 - 新增]
   ...
   - **⚠️ 重要**：如果用户请求"对X号患者进行影像分析/肿瘤检测/影像组学分析"，这是**单一的影像分析任务**，不是 multi_task！
     ...
     * 示例：
       - "对<case_id>号患者进行完整影像组学分析" → imaging_analysis（不是 multi_task！）
       - "检测<case_id>号病人的肿瘤" → imaging_analysis（不是 multi_task！）
       - "分析患者<case_id>的CT" → imaging_analysis（不是 multi_task！）
...
2. **pathology_analysis (病理分析)** [新增]
   ...
   - *示例*：
     - "对<case_id>号患者进行病理分析" → pathology_analysis
     - "用CLAM分析这张切片" → pathology_analysis
     - "查看<case_id>号病人的切片数据" → case_database_query
...
4. **case_database_query (查库/数据操作)**
   ...
   - *例外*：如果只是"查看<case_id>号患者信息"、"查询<case_id>号病历"等不包含影像关键词的请求，才选择 case_database_query
...
10. **multi_task (多任务复合命令)** [新增]
   - 用户在**一个命令中提出多个独立任务**（如"查询<case_id>号患者信息并给出治疗方案"、"分析影像然后制定手术方案"）。
   - 必须同时满足：
   ...
   - **⚠️ 常见误判 - 请注意以下场景不是 multi_task**：
     ...
     * "检测<case_id>号病人的肿瘤" → 这是 imaging_analysis，不是 multi_task！
     * "查询<case_id>号患者信息" → 这是 case_database_query，不是 multi_task！（只有一个任务）
...
{{"category": "multi_task", "sub_tasks": ["case_database_query", "treatment_decision"], "requires_context": true, "correction_suggestion": null, "reasoning": "用户要求查询<case_id>号患者信息并给出治疗方案，包含两个独立任务：先查询病例，再给出治疗建议"}}
