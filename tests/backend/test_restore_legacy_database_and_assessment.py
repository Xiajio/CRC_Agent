...
from src.graph_builder import route_after_assessment
...
def _make_workspace_tmp_dir() -> Path:
    base_dir = Path("tmp") / "backend-tests"
...
def _route_after_assessment_result(state: CRCAgentState, result: dict[str, object]) -> str:
    return route_after_assessment(state.model_copy(update=result))
...
def test_assessment_preserves_pathology_tnm_location_and_mmr_inquiry_chain() -> None:
    ...
    pathology_only_state = CRCAgentState(
        messages=[HumanMessage(content="术后病理提示腺癌")],
        encounter_track="crc_clinical",
        findings={"user_intent": "clinical_assessment"},
    )
    pathology_only = assessment(pathology_only_state)
    assert pathology_only["missing_critical_data"] == ["TNM Staging"]
    assert pathology_only["findings"]["active_inquiry"] is True
    assert pathology_only["findings"]["inquiry_type"] == "tnm_required"
    assert _route_after_assessment_result(pathology_only_state, pathology_only) == "end_turn"

    missing_location_state = CRCAgentState(
        messages=[HumanMessage(content="术后病理提示腺癌，cT3N1M0")],
        encounter_track="crc_clinical",
        findings={"user_intent": "clinical_assessment"},
    )
    missing_location = assessment(missing_location_state)
    assert missing_location["missing_critical_data"] == ["Tumor Location"]
    assert missing_location["findings"]["active_inquiry"] is True
    assert missing_location["findings"]["inquiry_type"] == "location_required"
    assert _route_after_assessment_result(missing_location_state, missing_location) == "end_turn"

    missing_mmr_state = CRCAgentState(
        messages=[HumanMessage(content="术后病理提示结肠腺癌，cT3N1M0")],
        encounter_track="crc_clinical",
        findings={"user_intent": "clinical_assessment"},
    )
    missing_mmr = assessment(missing_mmr_state)
    assert missing_mmr["missing_critical_data"] == ["MMR/MSI Status"]
    assert missing_mmr["findings"]["active_inquiry"] is True
    assert missing_mmr["findings"]["inquiry_type"] == "mmr_status_required"
    assert _route_after_assessment_result(missing_mmr_state, missing_mmr) == "end_turn"
...
def test_assessment_soft_case_summary_reports_tnm_gap_without_hard_blocking() -> None:
    assessment = node_assessment(model=_UnusedModel(), tools=[], show_thinking=False)

    result = assessment(
        CRCAgentState(
            messages=[HumanMessage(content="Pathology shows colon adenocarcinoma. Please draft a case summary.")],
            encounter_track="crc_clinical",
            findings={
    ...
    assert "TNM Staging" in result["findings"]["information_gaps"]
    assert result["findings"]["response_mode"] == "case_summary_with_gaps"
    assert result["findings"].get("fast_pass_mode") is not True
    assert result.get("patient_profile") is None


def test_assessment_soft_explanation_reports_location_gap_without_hard_blocking() -> None:
    assessment = node_assessment(model=_UnusedModel(), tools=[], show_thinking=False)

    result = assessment(
        CRCAgentState(
            messages=[HumanMessage(content="Pathology shows adenocarcinoma, cT3N1M0. Please explain what this means.")],
            encounter_track="crc_clinical",
            findings={
                "user_intent": "general_chat",
                "requires_complete_case": False,
    ...
    assert result["findings"]["active_inquiry"] is False
    assert result["findings"]["inquiry_type"] is None
    assert "Tumor Location" in result["findings"]["information_gaps"]
    assert result["clinical_stage"] != "Pending"


def test_assessment_soft_guidance_reports_mmr_gap_without_inquiry_pending() -> None:
    assessment = node_assessment(model=_UnusedModel(), tools=[], show_thinking=False)

    result = assessment(
        CRCAgentState(
            messages=[HumanMessage(content="Pathology shows colon adenocarcinoma, cT3N1M0. What information should I collect?")],
            encounter_track="crc_clinical",
            findings={
                "user_intent": "general_chat",
                "requires_complete_case": False,
    ...
    assert result["findings"]["active_inquiry"] is False
    assert result["findings"]["inquiry_type"] is None
    assert "MMR/MSI Status" in result["findings"]["information_gaps"]
