...
from src.nodes.node_utils import _invoke_with_streaming, clear_stream_callback, set_stream_callback
...
def test_invoke_with_streaming_emits_stable_message_id_and_matching_final_message() -> None:
    ...
    assert [event["type"] for event in callback_events] == ["start", "delta", "delta", "end"]
...
def test_invoke_with_streaming_only_emits_visible_response_deltas_and_preserves_thinking() -> None:
    ...
    assert [event["type"] for event in callback_events] == ["start", "delta", "end"]
