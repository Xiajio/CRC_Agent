...
import backend.api.services.graph_service as graph_service_module
...
@pytest.mark.asyncio
async def test_stream_turn_emits_case_database_cards_from_request_context(monkeypatch: pytest.MonkeyPatch) -> None:
    requested_patient_ids: list[int] = []

    def fake_case_detail(patient_id: int) -> dict[str, Any]:
        requested_patient_ids.append(patient_id)
        return {
            "patient_id": 93,
            "case_record": {
                "patient_id": 93,
            ...
            "cards": {
                "patient_card": {
                    ...
                    "text_summary": "Patient 093 demographics",
                },
                "imaging_card": {
                    "type": "imaging_card",
                    "data": {"images": [{"image_name": "ct.png"}]},
                    "text_summary": "Patient 093 imaging",
                },
                "pathology_slide_card": {
                    "type": "pathology_slide_card",
                    "data": {"folder_name": "093", "images": [{"image_name": "slide.png"}]},
                    "text_summary": "Patient 093 pathology slides",
                },
            },
        }

    monkeypatch.setattr(graph_service_module, "get_database_case_detail", fake_case_detail, raising=False)
    session_store = InMemorySessionStore()
    session = session_store.create_session(scene="doctor")
    ...
    events = await collect_sse_events(
        service.stream_turn(
            session.session_id,
            {
                **make_chat_request("lookup patient 93"),
                "context": {"case_database_patient_id": "93"},
            },
        )
    )

    card_events = [event for event in events if event["type"] == "card.upsert"]
    assert requested_patient_ids == [93]
    assert [event["card_type"] for event in card_events] == [
        "patient_card",
        "imaging_card",
        "pathology_slide_card",
    ]
    assert {event["source_channel"] for event in card_events} == {"state"}

    assert graph.last_payload is not None
    assert graph.last_payload["case_database_patient_id"] == "093"
    findings = graph.last_payload["findings"]
    assert findings["case_database_patient_id"] == "093"
    assert findings["data_source"] == "database_query"
    assert findings["db_query_patient_id"] == "093"
    assert findings["age"] == 58
    assert findings["gender"] == "female"
    assert findings["pathology_confirmed"] is True
    assert findings["tnm_staging"] == {"cT": "cT3", "cN": "cN1", "cM": "cM0"}

    card_payloads = {event["card_type"]: event["payload"] for event in card_events}
    assert graph.last_payload["patient_card"] == card_payloads["patient_card"]
    assert graph.last_payload["imaging_card"] == card_payloads["imaging_card"]
    assert graph.last_payload["pathology_slide_card"] == card_payloads["pathology_slide_card"]
    assert findings["patient_card"] == card_payloads["patient_card"]
    assert findings["imaging_card"] == card_payloads["imaging_card"]
    assert findings["pathology_slide_card"] == card_payloads["pathology_slide_card"]


@pytest.mark.asyncio
async def test_stream_turn_continues_without_case_cards_when_case_database_patient_is_missing(
    ...
    def fake_case_detail(patient_id: int) -> dict[str, Any]:
        requested_patient_ids.append(patient_id)
        raise graph_service_module.DatabaseCaseNotFoundError(f"Patient {patient_id} not found")
    ...
    events = await collect_sse_events(
        service.stream_turn(
            session.session_id,
            {
                **make_chat_request("lookup patient 999"),
                "context": {"case_database_patient_id": "999"},
            },
        )
    )

    assert requested_patient_ids == [999]
    assert "done" in [event["type"] for event in events]
    assert "error" not in [event["type"] for event in events]
    assert [event for event in events if event["type"] == "card.upsert"] == []
    assert graph.last_payload is not None
    assert graph.last_payload["case_database_patient_id"] == "999"
    assert graph.last_payload["findings"]["case_database_patient_id"] == "999"
...
@pytest.mark.asyncio
async def test_patient_graph_turn_caches_resolved_context_without_legacy_medical_card(
    tmp_path: Path,
) -> None:
    root = tmp_path / "test-graph-service-streaming"
    ...
    context_state = refreshed.context_state
    assert "medical_card" not in context_state
    cache = context_state["patient_context_cache"]
    assert cache["patient_id"] == patient.patient_id
    assert cache["medical_card_snapshot"] == legacy_card
    assert graph.payloads[0]["patient_context"]["patient_version"] == cache["patient_version"]
    assert graph.payloads[0]["medical_card"] == legacy_card
    done = find_event(parse_sse_payloads(chunks), "done")
    assert done["patient_version_used"] == cache["patient_version"]
    assert done["patient_context_stale"] is False
