...
def test_get_session_honors_message_limit_query(monkeypatch: pytest.MonkeyPatch) -> None:
    client, _store, _registry, _commands = _build_client(monkeypatch)
    create_response = client.post("/api/sessions", json={"scene": "doctor"})
    session_id = create_response.json()["session_id"]
    ...
    response = client.get(f"/api/sessions/{session_id}?message_limit=1")

    assert response.status_code == 200, response.text
    snapshot = response.json()["snapshot"]
    assert snapshot["messages_total"] == 2
    assert len(snapshot["messages"]) == 1
    assert snapshot["messages"][0]["content"] == "second"
